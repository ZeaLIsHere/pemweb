...
function generateInsights (sales, products) {
  ...
  const crossSellingAnalysis = analyzeCrossSellingOpportunities(recentSales, products)
  if (crossSellingAnalysis.insight) {
    insights.push(crossSellingAnalysis.insight)
  }

  // 11. Market Trend Prediction Algorithm
  const trendAnalysis = predictMarketTrends(todaySales, yesterdaySales, recentSales)
...
function analyzeSeasonalPatterns (monthlySales) {
  try {
    ...
    const peakDay = Object.keys(dayOfWeekData).reduce((a, b) => 
      dayOfWeekData[a] > dayOfWeekData[b] ? a : b, '0'
    )
...
function analyzePurchaseBehavior (recentSales, products) {
  try {
    if (recentSales.length < 5) return { insight: null }

    // Analyze purchase frequency
    const purchaseIntervals = []
    const sortedSales = recentSales.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp))
    
    for (let i = 1; i < sortedSales.length; i++) {
      const interval = new Date(sortedSales[i].timestamp) - new Date(sortedSales[i - 1].timestamp)
      purchaseIntervals.push(interval / (1000 * 60 * 60)) // Convert to hours
    }

    const avgInterval = purchaseIntervals.reduce((a, b) => a + b, 0) / purchaseIntervals.length
    
    // Analyze basket patterns
    const avgBasketSize = recentSales.reduce((sum, sale) => sum + (sale.totalItems || 1), 0) / recentSales.length
    ...
    if (avgInterval < 2) {
      ...
      recommendation = 'Pelanggan sering berbelanja! Tawarkan program loyalitas atau bundle deals.'
    } else if (avgInterval > 24) {
      behaviorType = 'occasional'
      recommendation = 'Pelanggan jarang berbelanja. Buat promosi menarik untuk meningkatkan frekuensi kunjungan.'
    } else {
      recommendation = 'Pola pembelian normal. Pertahankan kualitas layanan dan variasi produk.'
    }
    ...
    return {
      insight: {
        ...
        recommendation,
        priority: 'medium',
        data: { behaviorType, avgInterval: Math.round(avgInterval), avgBasketSize: Math.round(avgBasketSize) }
      }
    }
  } catch (error) {
    ...
    recentSales.forEach(sale => {
      ...
      if (product) {
        if (!pricePerformance[product.kategori]) {
          pricePerformance[product.kategori] = { sales: 0, revenue: 0, products: [] }
        }
...
function analyzeInventoryTurnover (recentSales, products) {
  try {
    const turnoverRates = products.map(product => {
      const productSales = recentSales.filter(sale => sale.productId === product.id).length
    ...
    const optimalTurnover = turnoverRates.filter(p => p.turnoverRate > 0.1 && p.turnoverRate < 0.8)
    const slowTurnover = turnoverRates.filter(p => p.turnoverRate < 0.1 && p.stok > 10)
    ...
    if (slowTurnover.length > 0) {
      return {
        insight: {
          ...
          recommendation: 'Produk dengan perputaran lambat sebaiknya dikurangi stoknya atau diberi promosi khusus untuk mempercepat penjualan.',
          priority: 'medium',
          data: { 
            slowProducts: slowTurnover.slice(0, 3).map(p => ({ nama: p.nama, rate: p.turnoverRate.toFixed(2) }))
          }
        }
      }
    }
    ...
    if (fastTurnover.length > 0) {
      return {
        insight: {
          ...
          recommendation: 'Produk dengan perputaran cepat perlu penambahan stok segera untuk menghindari kehabisan.',
          priority: 'high',
          data: { 
            fastProducts: fastTurnover.slice(0, 3).map(p => ({ nama: p.nama, rate: p.turnoverRate.toFixed(2) }))
          }
        }
      }
    }
...
function analyzeCrossSellingOpportunities (recentSales, products) {
  try {
    ...
    for (let i = 1; i < sortedSales.length; i++) {
      const timeDiff = new Date(sortedSales[i].timestamp) - new Date(sortedSales[i - 1].timestamp)
    ...
    const topCombo = Object.keys(combinations).reduce((a, b) => 
      ...
      if (product1 && product2) {
        return {
          insight: {
            ...
            priority: 'medium',
            data: { product1: product1.nama, product2: product2.nama, frequency: combinations[topCombo] }
          }
        }
      }
    }
...
function predictMarketTrends (todaySales, yesterdaySales, recentSales) {
  try {
    ...
    const weeklyTrend = dailyCounts.length > 1 ? 
      ((dailyCounts[dailyCounts.length - 1] - dailyCounts[0]) / dailyCounts[0]) * 100 : 0
    ...
    if (growthRate > 20) {
      ...
      recommendation = 'Tren penjualan sangat positif! Siapkan stok lebih banyak dan pertimbangkan ekspansi produk.'
      priority = 'high'
    } else if (growthRate > 5) {
      trendType = 'growth'
      recommendation = 'Penjualan meningkat. Pertahankan momentum dengan promosi yang tepat sasaran.'
      priority = 'medium'
    } else if (growthRate < -20) {
      trendType = 'decline'
      recommendation = 'Penjualan menurun drastis. Evaluasi strategi pemasaran dan cari tahu penyebabnya.'
      priority = 'high'
    } else if (growthRate < -5) {
      trendType = 'slight_decline'
      recommendation = 'Penjualan sedikit menurun. Lakukan promosi atau evaluasi harga produk.'
      priority = 'medium'
    } else {
      recommendation = 'Penjualan stabil. Fokus pada efisiensi operasional dan kepuasan pelanggan.'
    }
    ...
    return {
      insight: {
        ...
        recommendation,
        priority,
        data: { growthRate: growthRate.toFixed(1), weeklyTrend: weeklyTrend.toFixed(1), trendType }
      }
    }
  } catch (error) {
...
function analyzeProfitMargins (recentSales, products) {
  try {
    ...
    recentSales.forEach(sale => {
      ...
      if (product) {
        ...
        if (!categoryMargins[product.kategori]) {
          categoryMargins[product.kategori] = { margins: [], sales: 0, revenue: 0 }
        }
    ...
    const bestCategory = categoryAnalysis.reduce((a, b) => a.avgMargin > b.avgMargin ? a : b)
    const worstCategory = categoryAnalysis.reduce((a, b) => a.avgMargin < b.avgMargin ? a : b)
...
function forecastDemand (monthlySales, products) {
  try {
    ...
    monthlySales.forEach(sale => {
      const week = Math.floor((new Date() - new Date(sale.timestamp)) / (7 * 24 * 60 * 60 * 1000))
    ...
    Object.keys(weeklyDemand).forEach(productId => {
      const weeks = Object.keys(weeklyDemand[productId]).map(Number).sort((a, b) => b - a)
      if (weeks.length >= 3) {
...
function assessBusinessRisks (recentSales, products) {
  try {
    ...
    if (dailyCounts.length > 2) {
      ...
      const variance = dailyCounts.reduce((sum, count) => sum + Math.pow(count - avg, 2), 0) / dailyCounts.length
