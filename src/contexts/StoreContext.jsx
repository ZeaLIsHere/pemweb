...
export function StoreProvider ({ children }) {
  ...
  const [storeStats, setStoreStats] = useState({ totalSales: 0, totalRevenue: 0, todaySales: 0, todayRevenue: 0 })
  ...
  useEffect(() => {
    if (!currentUser?.uid) {
      setStoreStats({ totalSales: 0, totalRevenue: 0, todaySales: 0, todayRevenue: 0 })
      return
    }

    const unsubscribe = storeStatsService.getStoreStats(currentUser?.uid, (stats) => {
      setStoreStats(stats)
